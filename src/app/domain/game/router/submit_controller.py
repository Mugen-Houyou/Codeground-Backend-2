...
async def submit_code(
    request: schemas.SubmitRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """
    ...
    event_generator = service.stream_evaluate_code(
        db,
...
async def submit_code_public(request: schemas.SubmitRequest, db: Session = Depends(get_db)):
    """
